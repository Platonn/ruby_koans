...
class AboutIteration < Neo::Koan
  ...
  def test_iterating_with_each
    array = [1, 2, 3]
    sum = 0
    array.each do |item| # W |......| kwantyfikujemy zmienne w bloku
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind_MOJ_TEST
    result = [2, 3, 4].inject(2) { |sum, item| sum * item }
